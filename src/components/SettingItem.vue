    ...
    <div @click="item.show = !item.show" class="myCardHeaderBlock">
      <div class="settings__login">
        ...
        <div class="settings__login-arrow">
          <svg-arrow-up
            :style="{
              transform: !item.show ? 'rotateX(180deg)' : 'rotateX(0deg)',
            }"
            class="commission"
          />
        </div>
          ...
          <div class="settings__activeLogin" v-if="item.type != 'file'">
            ...
            <span>{{
              item.name === "birthday" && user[item.name]
                ? user[item.name].substring(0, 10)
                : user[item.name]
            }}</span>
          </div>
          <div>
            <div
              class="settings__login_edit"
              v-if="item.type != 'radio' && item.type != 'file'"
            >
              <input
                v-model="item.value"
    ...
    async change() {
      if (this.item.type == "file") {
        ...
        reader.onload = async (e) => {
          ...
          await this.$store.dispatch("updateUserPhoto", {
            id: this.user.id,
            photo: image,
          });
        };
        ...
        await this.$store.dispatch("updateUser", {
          id: this.user.id,
          newParam: { [this.item.name]: this.item.value },
        });
      }
    },
  },
